...
`n4` Form XObject structure. The safe default renders an unknown mark and `SIGNATURE NOT VERIFIED`
in the `n4` status layer:
...
An application that has independently verified the claim can render the green check and signed-valid
text:
...
The other static state is `invalid`. Use `markColor`, `statusTextColor`, or `showMark` to customize
the status artwork.

<Callout type="warn" title="Visual status is not verification">
  The n1/n3 mark and n4 text are static appearance content. They do not verify the cryptographic
  signature, establish certificate trust, or control the validity indicator shown by Adobe Acrobat
