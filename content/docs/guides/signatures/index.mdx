...
### OpenPDF-Compatible n0-n4 Layers

Set `legacyLayers: true` to wrap the normal appearance in the historical `FRM` and `n0` through
`n4` Form XObject structure. It renders an unverified placeholder in `n1`, leaves `n3` blank for
viewer-managed validity, and writes `Signature Not Verified` in `n4`:
...
Use `statusText` or `statusTextColor` to customize the `n4` placeholder. The `n1` `DSUnknown`
template intentionally matches OpenPDF's legacy stream.

<Callout type="warn" title="Validity belongs to the viewer">
  A green check or other validated status is calculated by the PDF viewer from document integrity,
  certificate trust, time, and revocation information. The library intentionally does not author a
  static valid state because appearance content cannot establish signature validity.
</Callout>
